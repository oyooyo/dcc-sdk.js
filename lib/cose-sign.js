...
import common from './cose-common';
...
function doSignSync(SigStructure, signer, alg) {
  if (!AlgFromTags[alg]) {
    throw new Error('Unknown algorithm, ' + alg);
  }
  if (!COSEAlgToNodeAlg[AlgFromTags[alg].sign]) {
...
export function createSync(headers, payload, signers, options) {
  options = options || {};
  let u = headers.u || {};
  let p = headers.p || {};
  p = common.TranslateHeaders(p);
  ...
  if (Array.isArray(signers)) {
    ...
    const sig = doSignSync(SigStructure, signer, alg);
    if (p.size === 0 && options.encodep === 'empty') {
      p = EMPTY_BUFFER;
    } else {
      p = cbor.encode(p);
...
function doVerifySync (SigStructure, verifier, alg, sig) {
  if (!AlgFromTags[alg]) {
    throw new Error('Unknown algorithm, ' + alg);
  }
  if (!COSEAlgToNodeAlg[AlgFromTags[alg].sign]) {
...
export function verifySync(payload, verifier, options) {
  options = options || {};
  ...
  if (type === SignTag) {
    ...
    doVerifySync(SigStructure, verifier, alg, sig);
    return plaintext;
  } else {
    const externalAAD = verifier.externalAAD || EMPTY_BUFFER;
    ...
    doVerifySync(SigStructure, verifier, alg, signer);
    return plaintext;
  }
};
