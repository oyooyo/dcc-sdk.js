...
import cose from 'cose-js/lib/sign.js'
...
async function verifyAndReturnPayload(coseContent, addPublicKeyPem) {
  ...
  // Tries B64URL First
  let publicKeyPem = resolveKey(toBase64URL(keyID.kid));

  // if not found then regular Base64
  if (!publicKeyPem) {
    publicKeyPem = resolveKey(toBase64(keyID.kid));
  }
...
export async function unpackAndVerify(uri, publicKeyPem) {
  try {
    return await verifyAndReturnPayload(unpack(uri), publicKeyPem);
  } catch (err) {
    console.log(err);
    return undefined;
  }
...
export async function signAndPack32(payload, publicKeyPem, privateKeyP8) {
  return pack32(await sign(payload, publicKeyPem, privateKeyP8));
}

export async function signAndPack45(payload, publicKeyPem, privateKeyP8) {
  return pack45(await sign(payload, publicKeyPem, privateKeyP8));
}
